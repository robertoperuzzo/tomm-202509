...
class TypesenseIndexer(BaseIndexer):
    ...
    def index_all_combinations(self, max_documents: int = -1,
        ...
        for extraction_method in extraction_methods:
            ...
            files = self.data_processor.get_processed_files(extraction_method, 1)
            if not files:
            ...
            strategies = self.data_processor.get_available_strategies(chunks_data)
            logger.info(f"Found strategies for {extraction_method}: {strategies}")
